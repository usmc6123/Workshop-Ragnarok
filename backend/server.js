...
// Video editor routes
let videoEditorRouter;
try {
  videoEditorRouter = require('./video-editor-routes');
} catch (e) {
  console.warn('[Server] Could not require separate video-editor-routes.js, creating fallback inline router:', e.message);
  ...
  const scheduleRenderJobCleanup = (jobId, delayMs = 10 * 60 * 1000) => {
  ...
  fallbackRouter.delete('/:id', (req, res) => {
    try {
      const { id } = req.params;
      const project = db.prepare('SELECT * FROM video_projects WHERE id = ? AND user_id = ?').get(id, req.user.id);
      if (!project) return res.status(404).json({ error: 'Video project not found' });
  ...
  fallbackRouter.post('/:id/render', async (req, res) => {
    try {
      const { id } = req.params;
      const project = db.prepare('SELECT * FROM video_projects WHERE id = ? AND user_id = ?').get(id, req.user.id);
      if (!project) return res.status(404).json({ error: 'Video project not found' });
      ...
      if (clips.length === 0) return res.status(400).json({ error: 'Cannot render project with no clips.' });
      ...
      for (const clip of clips) {
        ...
        if (clip.source_url.startsWith('/uploads/')) {
          ...
          if (!fs.existsSync(localPath)) localPath = path.join(UPLOADS_ROOT, basename);
        }
        let durationSec = 0;
        try { durationSec = await probeDurationSeconds(localPath); } catch (e) {}
        ...
        clipInfos.push({ ...clip, localPath, start, end, clipDur, totalDur: durationSec });
      }
      if (totalDuration === 0) return res.status(400).json({ error: 'Total duration is 0, cannot render.' });
      ...
      (async () => {
        try {
          const args = ['-y'];
          clipInfos.forEach(info => { args.push('-i', info.localPath); });
          ...
          if (bgMusic && bgMusic.source_url) {
            ...
            if (bgMusic.source_url.startsWith('/uploads/')) {
              ...
              if (!fs.existsSync(musicLocalPath)) musicLocalPath = path.join(UPLOADS_ROOT, basename);
            }
          ...
          imageOverlays.forEach((ov, idx) => {
            ...
            if (ov.image_url.startsWith('/uploads/')) {
              ...
              if (!fs.existsSync(imgLocalPath)) imgLocalPath = path.join(UPLOADS_ROOT, basename);
            }
            if (!fs.existsSync(imgLocalPath)) {
              const pubPath = path.join(process.cwd(), 'public', ov.image_url.replace(/^\//, ''));
              if (fs.existsSync(pubPath)) imgLocalPath = pubPath;
            }
          ...
          const encodeTimeoutMs = Math.min(90 * 60 * 1000, Math.max(15 * 60 * 1000, durationMs * 4));
          await new Promise((resolve, reject) => {
            ...
            const timeoutHandle = setTimeout(() => { proc.kill('SIGKILL'); reject(new Error('Render timed out.')); }, encodeTimeoutMs);
            proc.on('error', (error) => { clearTimeout(timeoutHandle); reject(error); });
            proc.on('close', (code) => {
              clearTimeout(timeoutHandle);
              if (code !== 0) return reject(new Error('Video render failed — ffmpeg process exited with code ' + code));
              resolve();
            });
          });
          ...
          db.prepare(`UPDATE video_projects SET status = 'rendered', output_url = ?, updated_at = datetime('now') WHERE id = ?`).run(outputUrl, id);
          renderJobs.set(jobId, { status: 'done', percent: 100, url: outputUrl });
        } catch (error) {
          db.prepare(`UPDATE video_projects SET status = 'error', updated_at = datetime('now') WHERE id = ?`).run(id);
          renderJobs.set(jobId, { status: 'error', percent: 0, error: error.message || 'Render failed' });
        } finally {
  ...
  fallbackRouter.get('/:id/render/:jobId', (req, res) => {
...
try {
  ...
  try {
    ...
    if (!funnelsCols.some(c => c.name === 'headline_bg_image_url')) {
      db.exec(`ALTER TABLE funnels ADD COLUMN headline_bg_image_url TEXT`);
      console.log('Successfully migrated funnels to include headline_bg_image_url column.');
    }
    if (!funnelsCols.some(c => c.name === 'secondary_video_url')) {
      db.exec(`ALTER TABLE funnels ADD COLUMN secondary_video_url TEXT`);
      console.log('Successfully migrated funnels to include secondary_video_url column.');
    }
    if (!funnelsCols.some(c => c.name === 'headline_bg_video_url')) {
      db.exec(`ALTER TABLE funnels ADD COLUMN headline_bg_video_url TEXT`);
      console.log('Successfully migrated funnels to include headline_bg_video_url column.');
    }
    ...
    if (!funnelsCols.some(c => c.name === 'hero_video_url')) {
      db.exec(`ALTER TABLE funnels ADD COLUMN hero_video_url TEXT`);
      console.log('Successfully migrated funnels to include hero_video_url column.');
    }
    if (!funnelsCols.some(c => c.name === 'video_form_bg_image_url')) {
  ...
  try {
    ...
    `);
    console.log('Successfully initialized video_projects table.');
  } catch (err) {
    console.error('Error creating video_projects table:', err);
  }
